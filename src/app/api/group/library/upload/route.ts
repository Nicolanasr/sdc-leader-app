  ...
  try {
    ...
    if (!allowedRoles.includes(currentRole)) {
      return NextResponse.json(
        { error: 'Forbidden. Upload access is restricted to Group Leaders and Secretary.' },
        { status: 403 }
      )
    }
    ...
    if (youtubeUrl) {
      ...
      // Folder nesting: Root -> Group Name -> Scout Library -> Category Name
      ...
      // Convert File stream to buffer for Google Drive
    ...
    if (dbError) {
      ...
      return NextResponse.json({ error: dbError.message || 'Failed to save item metadata.' }, { status: 500 })
    }
    ...
    console.error('[Library Upload API] Exception:', err)
    return NextResponse.json(
      { error: err.message || 'Internal server error while uploading library item.' },
      { status: 500 }
    )
  }
}
