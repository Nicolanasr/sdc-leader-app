...
import {
    ...
    Smartphone, Copy, Check, Grid, Loader2, Tent
} from 'lucide-react'
...
export default function EventsManagement({
    ...
    return (
        <DashboardShell groupName={groupName} currentRole={currentRole} userName={userName}>
            ...
            {/* ── NATIVE MOBILE-FIRST HEADER & CONTROLS ── */}
            <div className="bg-white p-3 sm:p-4 rounded-2xl border border-slate-200/90 shadow-2xs space-y-2.5 mb-2">
                {/* Row 1: Title & Subtitle + Desktop Controls */}
                <div className="flex items-center justify-between gap-2.5">
                    <div className="flex items-center gap-2.5 min-w-0">
                        <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-teal-50 border border-teal-200 flex items-center justify-center text-teal-800 shrink-0 shadow-2xs">
                            <Tent className="h-4 w-4 sm:h-5 sm:w-5" />
                        </div>
                        <div className="min-w-0">
                            ...
                            <p className="text-[10px] sm:text-[11px] text-slate-500 truncate">
                                Camps, hikes, activities & calendar schedule
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="hidden sm:flex items-center gap-2 shrink-0">
                        <button
                            ...
                            className="bg-amber-50 hover:bg-amber-100 active:scale-95 text-amber-900 font-bold px-2.5 py-1.5 rounded-xl text-xs border border-amber-200 transition-all flex items-center gap-1 shadow-2xs"
                        >
                        ...
                        <div className="bg-slate-100/90 p-0.5 rounded-xl flex items-center gap-0.5 border border-slate-200/70">
                            <button
                                ...
                                className={`flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-bold transition-all ${viewMode === 'grid' ? 'bg-teal-800 text-white shadow-2xs' : 'text-slate-600 hover:text-slate-900'}`}
                            >
                                <Grid className="h-3.5 w-3.5" /> List
                            </button>
                            <button
                                onClick={() => setViewMode('calendar')}
                                className={`flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-bold transition-all ${viewMode === 'calendar' ? 'bg-teal-800 text-white shadow-2xs' : 'text-slate-600 hover:text-slate-900'}`}
                            >
                ...
                {/* Row 2 (Mobile Only): Touch Action Strip */}
                <div className="flex sm:hidden items-center justify-between gap-1.5 pt-0.5">
                    <div className="bg-slate-100/90 p-0.5 rounded-xl flex items-center gap-0.5 border border-slate-200/70 flex-1">
                        <button
                            onClick={() => setViewMode('grid')}
                            className={`flex-1 flex items-center justify-center gap-1 py-1 rounded-lg text-[11px] font-bold transition-all ${viewMode === 'grid' ? 'bg-teal-800 text-white shadow-2xs' : 'text-slate-600'}`}
                        >
                            <Grid className="h-3 w-3" /> List
                        </button>
                        <button
                            onClick={() => setViewMode('calendar')}
                            className={`flex-1 flex items-center justify-center gap-1 py-1 rounded-lg text-[11px] font-bold transition-all ${viewMode === 'calendar' ? 'bg-teal-800 text-white shadow-2xs' : 'text-slate-600'}`}
                        >
                    ...
                    <button
                        onClick={() => setIsSyncModalOpen(true)}
                        className="bg-amber-50 active:bg-amber-100 text-amber-900 font-bold px-2 py-1 rounded-xl text-[11px] border border-amber-200 transition-all flex items-center gap-1 shadow-2xs active:scale-95"
                    >
                    ...
                    <button
                        onClick={() => { resetForm(); setIsCreateModalOpen(true) }}
                        className="bg-teal-800 active:bg-teal-900 text-white font-bold px-2.5 py-1 rounded-xl text-[11px] transition-all shadow-2xs flex items-center gap-1 active:scale-95 shrink-0"
                    >
