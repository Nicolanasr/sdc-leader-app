...
export default function MembersManagement({
  ...
  const handleSaveMember = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!firstName || !lastName || !emergencyName || !emergencyRelation || !emergencyPhone) {
      return showStatus('Please fill in Name and Emergency Contact details.', 'error')
    }
    ...
    <div className="flex h-screen bg-slate-50 text-slate-900 overflow-hidden relative">
      ...
      <main className="flex-1 overflow-y-auto flex flex-col">
        ...
        <div className="p-6 md:p-8 flex-1 flex flex-col">
          ...
          <div className="flex justify-between items-center mb-6">
            <div>
              <h2 className="text-3xl font-extrabold tracking-tight text-slate-900">Youth Roster</h2>
            ...
            {canWrite && (
              <div className="flex gap-3">
                <button
                  ...
                  className="inline-flex items-center gap-2 bg-slate-100 hover:bg-slate-200 text-slate-800 font-semibold px-4 py-2.5 rounded-xl border border-slate-350 transition-colors text-sm"
                >
          ...
          {/* Filters Bar */}
          <div className="bg-white border border-slate-200 p-4 rounded-2xl shadow-sm mb-6 flex flex-col md:flex-row gap-4 items-center justify-between">
            <div className="relative w-full md:max-w-xs">
              ...
              <input
                ...
                className="pl-9 pr-4 py-2 w-full rounded-lg border border-slate-255 bg-white text-sm focus:border-teal-500 focus:outline-none"
              />
            </div>
            <div className="flex flex-wrap gap-4 w-full md:w-auto">
              {!isTroopLeader && (
                <select
                  ...
                  className="rounded-lg border border-slate-255 bg-white text-sm px-3 py-2 text-slate-700"
                >
          ...
          <div className="flex-1 grid grid-cols-1 xl:grid-cols-3 gap-8 items-start">
            ...
            <div className="xl:col-span-2 bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse min-w-[500px]">
                  <thead>
                    <tr className="border-b border-slate-200 bg-slate-50 text-slate-550 text-xs font-bold uppercase">
                      <th className="px-6 py-3">Scout Name</th>
                      <th className="px-6 py-3">Rank</th>
                      <th className="px-6 py-3">Assigned Unit</th>
                      <th className="px-6 py-3">Patrol Group</th>
                      ...
                      filteredMembers.map((member) => {
                        ...
                        return (
                          <tr key={member.id} className="hover:bg-slate-50/50 cursor-pointer" onClick={() => setSelectedMember(member)}>
                            ...
                            <td className="px-6 py-4 text-xs font-semibold text-teal-850">
            ...
            {/* Profile detail card */}
            {selectedMember && (
              <div className="bg-white border border-slate-200 rounded-2xl shadow-md p-6 space-y-6 relative self-start">
                <button
                ...
                <div>
                  <h3 className="text-xl font-bold text-slate-900">{selectedMember.first_name} {selectedMember.last_name}</h3>
                  <p className="text-xs text-slate-400 mt-1">Scout Registration ID: {selectedMember.id}</p>
                </div>

                <div className="grid grid-cols-2 gap-4 text-xs">
                  <div>
                    <span className="block text-slate-400 uppercase font-semibold">Blood Type</span>
                    <span className="font-bold text-rose-600 text-sm mt-0.5 flex items-center gap-1">
                      <Heart className="h-4 w-4 fill-rose-600" />
                      {selectedMember.blood_type || 'N/A'}
                    </span>
                  </div>
                  <div>
                    <span className="block text-slate-400 uppercase font-semibold">Birth Date</span>
                    <span className="font-bold text-slate-800 mt-0.5 block">{selectedMember.birth_date || 'N/A'}</span>
                  </div>
                </div>
                ...
                <div className="border-t border-slate-100 pt-4 space-y-2">
                  ...
                  <div className="bg-slate-50 border border-slate-150 p-3 rounded-xl text-xs space-y-1.5">
                    <div className="flex justify-between">
                      ...
                      <span className="font-semibold text-slate-850">{selectedMember.emergency_contact_name}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-550">Relation:</span>
                      <span className="font-semibold text-slate-850">{selectedMember.emergency_contact_relation}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-550">Phone Number:</span>
                      <span className="font-bold text-teal-850">{selectedMember.emergency_contact_phone}</span>
                    </div>
                  </div>
                </div>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs overflow-y-auto">
          <div className="bg-white w-full max-w-2xl p-6 border border-slate-200 rounded-2xl shadow-xl space-y-4 my-8">
            ...
            <form onSubmit={handleSaveMember} className="space-y-6">
              ...
              <div className="border-t border-slate-100 pt-4 space-y-4">
                <h4 className="text-sm font-bold text-teal-850">Emergency Contact Details</h4>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
